...
@section('title', 'Génération d\'annonces IA')

@section('content')
<div class="max-w-7xl mx-auto py-10">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Génération d'annonces IA</h1>
            <p class="text-gray-600 mt-2">Créez des annonces optimisées SEO avec l'intelligence artificielle</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Configuration de la génération</h2>
        </div>
        
        <form method="POST" action="{{ route('admin.ads.generation.generate') }}" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <!-- Type de génération -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Type de génération</label>
                        <select name="generation_type" id="generation-type" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                            <option value="keyword_cities">Mots-clés + Villes</option>
                            <option value="service_cities">Service + Villes</option>
                            <option value="seo_articles">Articles SEO</option>
                        </select>
                    </div>

                    <!-- Mots-clés (conditionnel) -->
                    <div id="keywords-field" style="display: none;">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Mots-clés SEO</label>
                        <input type="text" name="keywords" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: couvreur, toiture, réparation, devis">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Afficher/masquer le champ mots-clés
    generationType.addEventListener('change', function() {
        if (this.value === 'keyword_cities') {
    ...
    function loadFavoriteCities() {
        fetch('/admin/ads/generation/favorite-cities')
    ...
    function loadCitiesByRegion(region) {
        fetch(`/admin/ads/generation/cities-by-region?region=${encodeURIComponent(region)}`)
            .then(response => response.json())
            .then(data => {
                filteredCities = data.cities;
                displayCities(data.cities);
