...
class AdGenerationPageController extends Controller
{
    /**
     * Afficher la page de génération d'annonces
     */
    public function index()
    {
        ...
        return view('admin.ads.generation', compact(
            'services', 
            'favoriteCities', 
            'regions', 
            'totalCities', 
    ...
    /**
     * Générer des annonces via IA
     */
    public function generate(Request $request)
    {
        $request->validate([
            'service_slug' => 'required|string',
            'generation_type' => 'required|in:keyword_cities,service_cities,seo_articles',
            ...
            foreach ($cities as $city) {
                for ($i = 0; $i < $countPerCity; $i++) {
                    try {
                        ...
                        $aiContent = $this->generateAIContent($service, $city, $keywords, $generationType);
     ...
     * Générer le contenu via IA
     */
    private function generateAIContent($service, $city, $keywords = null, $generationType = 'keyword_cities')
    {
        ...
        $prompt = $this->buildPrompt($service, $city, $keywords, $generationType);
     ...
     * Construire le prompt selon le type de génération
     */
    private function buildPrompt($service, $city, $keywords, $generationType)
    {
        $basePrompt = "Génère une annonce SEO optimisée pour {$service['name']} à {$city->name} ({$city->postal_code}).";
        
        switch ($generationType) {
            case 'keyword_cities':
                $prompt = $basePrompt . " Utilise ces mots-clés : {$keywords}. ";
                break;
                
            case 'service_cities':
                $prompt = $basePrompt . " Focus sur le service {$service['name']} dans la région {$city->region}. ";
                break;
                
            case 'seo_articles':
                $prompt = $basePrompt . " Crée un article de blog SEO sur {$service['name']} à {$city->name}. ";
                break;
