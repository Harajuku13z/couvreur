...
/**
 * Correspondance nom de département ↔ code INSEE (comme en admin villes).
 */
class FrenchDepartments
{
    /** @var array<string, string> nom officiel => code */
    ...
    public static function nameFromCode(?string $code): ?string
    {
        ...
        $n = self::normalizeCode($code);

        return self::codeToNameMap()[$n] ?? null;
    }

    /**
     * @return array<string, string> code => nom
     */
    public static function codeToNameMap(): array
    {
        if (self::$codeToName === null) {
            self::$codeToName = array_flip(self::NAME_TO_CODE);
        }
